...
test('feature-workbench：组合 P0 planner/workspace/review 槽位给桌面宿主', () => {
  ...
  assert.equal(module.slots.planner.name, 'planner-sidebar');
  assert.deepEqual(module.slots.planner.props, {
    activeSection: 'today',
    currentDate: '2026-02-10',
    intent: 'Ship the desktop loop',
    summary: '1 active projects · 3 open tasks · 2 for today · 4 review signals'
  });
  assert.deepEqual(module.slots.workspace.props, {
    projectCount: 2,
    taskCount: 4,
    todayCount: 2,
    focusTaskId: 'task-focus'
  });
  assert.deepEqual(module.slots.review.props, {
    reviewItemCount: 4,
...
test('feature-workbench：提供宿主可调用的 P0 挂载结果', () => {
  ...
  assert.equal(rerendered.slots.review.name, 'review-panel');
});
