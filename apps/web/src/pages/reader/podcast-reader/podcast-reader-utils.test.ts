...
import {
  ...
  resolvePodcastAboutContent,
} from './podcast-timestamps';
import { getPodcastOutlineTitleClassName } from './podcast-outline';
...
describe('podcast timestamp lines', () => {
  ...
  it('treats real html show notes as html content', () => {
    expect(resolvePodcastAboutContent('<p>00:10 开场</p><p>第二段</p>', '00:10 开场\n第二段')).toEqual({
      mode: 'html',
      value: '<p>00:10 开场</p><p>第二段</p>',
    });
  });

  it('treats plain text content stored in content as text instead of html', () => {
    expect(resolvePodcastAboutContent('00:10 开场\n\n第二段', null)).toEqual({
      mode: 'text',
      value: '00:10 开场\n\n第二段',
    });
  });
...
describe('podcast outline rendering', () => {
  it('allows timeline titles to wrap to any number of lines without ellipsis', () => {
    const className = getPodcastOutlineTitleClassName();
