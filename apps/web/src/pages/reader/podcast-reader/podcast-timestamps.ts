...
export interface ResolvedPodcastAboutContent {
...
function normalizeContentValue(value: string | null | undefined): string | null {
...
export function resolvePodcastAboutContent(
  content: string | null | undefined,
  contentText: string | null | undefined,
): ResolvedPodcastAboutContent {
  const normalizedContent = normalizeContentValue(content);
  if (normalizedContent && looksLikeHtmlContent(normalizedContent)) {
    return { mode: 'html', value: normalizedContent };
  }

  const normalizedText = normalizedContent ?? normalizeContentValue(contentText);
  if (normalizedText) {
    return { mode: 'text', value: normalizedText };
  }
