...
## 6. CLI 作为能力适配器与上下文策略补充

### 6.1 背景：关于 CLI vs MCP vs SKILL 的行业讨论

2025–2026 年间，社区出现了两类有代表性的观点：

1. **"CLI-native" 派**：认为 MCP 对日常开发工作流是 over-engineering，coding agent 直接使用 `git`、`rg`、`curl`、`jq` 等成熟 CLI 工具 + shell pipe 即可完成绝大多数任务，且 token 效率更高、模型对 shell 的理解更好。
2. **"MCP + SKILL" 派**：承认 MCP 的上下文预算问题（工具 schema 挤占 context），提出 SKILL（场景化工作流封装）+ uxc（统一协议 CLI）作为 MCP 的补充层，核心洞察是"上下文适合承载决策信息，而不是当作数据存放层"。

### 6.2 Orbit 的定位决定了 MCP 不可替代

两种观点的共同前提是：agent 作为**外部操作者**，操作的对象是文件系统、进程、网络上已有的、标准化的资源。这些资源已有成熟 CLI 接口，MCP 在此场景下确实是多余的一层。

Orbit 面临的是根本不同的问题：**Orbit 自身就是被操作的对象系统**。`task.compute_next`、`research.source_bundle.get`、`writing.create_draft_from_brief` 这些能力没有对应的现成 CLI 工具，因为它们是 Orbit 自己定义的产品语义。Orbit 同时扮演 MCP Server（暴露能力给外部 agent）和 MCP Client（接入外部服务），MCP 是与外部 agent 生态互操作的标准协议，不能跳过。

此外，Orbit 的能力治理（scope 校验、审批、审计、数据出境控制）是产品级需求，无法用 shell pipe 表达。文章中"Bash is the ultimate MCP"的论断对 Orbit 不成立。

### 6.3 CLI 作为 Capability Runtime 的第四个适配器

尽管 MCP 不可替代，CLI 仍值得作为 Capability Runtime 的**补充入口**。当前设计中 Capability Runtime 已有三个消费路径（UI、应用内 Agent、外部 MCP client），CLI 作为第四个适配器有独特价值：

| 场景 | CLI 的优势 |
|---|---|
| **开发调试** | `orbit capability list --domain=research` 比打开 MCP inspector 快 |
| **脚本集成** | 用户的 shell 脚本、cron job 可直接调用 |
| **Coding agent 集成** | Claude Code / Amp 等 agent 可通过 `orbit task next --json` 读取上下文，不需要配置 MCP 连接 |
| **渐进式披露** | `orbit -h` → `orbit task -h` → `orbit task next` 的层级探索比一次性暴露全部 MCP schema 更友好 |

CLI 与 MCP 的关系是**同一 Capability Runtime 的不同适配器**，共享同一套策略引擎：

```
Actor (UI / Internal Agent / MCP Client / CLI)
  → Capability Runtime
    → Policy Engine → Approval Engine → Handler → Audit Sink
```

CLI 适配器的设计原则：

1. **不绕过治理**：CLI 调用与 MCP 调用经过相同的 Policy → Approval → Handler → Audit 链路。
2. **结构化输出**：默认输出 JSON，方便 agent 解析；`--human` flag 切换人类可读格式。
3. **渐进式披露**：顶层只列域，子命令展开具体 capability，避免 help 信息过长。
4. **凭证外部化**：认证信息走系统 keychain 或配置文件，不出现在命令参数中。

### 6.4 上下文预算与结果外部化策略

"MCP + SKILL" 派提出的核心洞察——**上下文适合承载决策信息，而不是当作数据存放层**——与 Orbit 的现有设计高度吻合，但值得显式记录为设计原则：

**原则：Capability 调用结果应按长度和类型分流，而非一律回填 agent 上下文。**

| 结果类型 | 处理方式 | 回填内容 |
|---|---|---|
| 短结果（< 2K tokens） | 直接回填 | 完整结果 |
| 长结果（摘要、报告、证据包） | 落入对象层（`artifact`、`draft`、`snapshot`） | `ObjectRef` + 摘要 |
| 流式结果（搜索、列表） | 落入临时资源，支持分页 | 前 N 条 + `Cursor` |
| 二进制/附件 | 落入文件系统 | 文件路径 + 元数据摘要 |

这与 doc 14 的记忆分层一致：L0 Turn Scratch 只保留当前回合的决策信息，L2 Object Memory 承载结构化事实。Capability Runtime 在返回结果时应自动执行此分流，而不是让每个 capability handler 自行决定。

**对 MCP Server 暴露策略的补充**：Orbit 作为 MCP Server 时，`tools/list` 应支持分域过滤。外部 agent 首次连接时只看到域列表和高频 capability，需要时再通过 `tools/list?domain=research` 展开具体工具。这借鉴了 uxc 的渐进式披露思路和 Claude Code 的 `searchHint` / `alwaysLoad` 机制，但不需要引入额外的中间层——直接在 MCP Server 适配器中实现即可。
...
8. **CLI 不是 MCP 的替代，而是同一 Capability Runtime 的另一个适配器**：CLI 的便利性不能成为绕过治理链路的理由；CLI 与 MCP 共享 Policy → Approval → Handler → Audit 全链路。
